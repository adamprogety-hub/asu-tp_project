...
const policySections = [
  ["01", "Общие положения", "Настоящая Политика определяет порядок обработки и защиты персональных данных посетителей сайта AERON. Оператор обрабатывает данные законно, добросовестно и только для заранее определённых целей."],
...
export function PrivacyModal() {
  ...
  return <div className="privacy-modal-layer" role="dialog" aria-modal="true" aria-labelledby="privacy-title" onClick={()=>setOpen(false)}>
    <div className="privacy-modal" onClick={(event)=>event.stopPropagation()}>
      <header><div><span>Правовая информация · Редакция 03.08.2026</span><h2 id="privacy-title">Политика конфиденциальности</h2><p>Обработка персональных данных и использование файлов cookie на сайте AERON.</p></div><button type="button" onClick={()=>setOpen(false)} aria-label="Закрыть политику"><X/></button></header>
      <div className="privacy-modal-body">
        <div className="privacy-operator"><strong>Реквизиты оператора перед публикацией</strong><p>Необходимо указать полное наименование ИП или юридического лица, ИНН, юридический адрес и рабочий email. Сейчас оператор обозначен как владелец сайта AERON.</p></div>
      ...
      <footer><span>AERON · Диспетчеризация вентиляции</span></footer>
    </div>
  </div>;
}
