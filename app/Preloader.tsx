...
/**
 * Cinematic preloader:
 * 1. Light paper screen, logo folds in path-by-path (origami effect)
 * 2. After 1.7s logo settles + star pulses
 * 3. After 2.1s total → Curtain Up: preloader slides upward, hero revealed
 *
 * Shown only once per session (sessionStorage flag).
 */
export function Preloader() {
  const [phase, setPhase] = useState<"idle" | "show" | "exit" | "done">("idle");

  useEffect(() => {
    // Skip on repeated visits within same tab session
    ...
    const doneTimer = setTimeout(() => {
      ...
      setPhase("done");
    }, 3000); // exit animation = 0.85s, so done at 2.1 + 0.9 = 3s
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      {/* 6-path logo — each folds in separately */}
      <svg
        className="pl-logo"
